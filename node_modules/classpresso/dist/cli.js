import { readFile, writeFile, appendFile, stat, copyFile } from 'fs/promises';
...
async function findFiles(buildDir, patterns) {
  const globPatterns = [...new Set(patterns.map((p) => path3.join(buildDir, p)))];
...
async function loadConfig(buildDir) {
  ...
  for (const configPath of configPaths) {
    try {
      const fullPath = path3.resolve(process.cwd(), configPath);
...
function getVersion() {
  try {
    ...
    const packageJsonPath = path3.resolve(__dirname, "../package.json");
...
function createLogger(options) {
  ...
  const logPath = debug ? path3.join(buildDir, DEBUG_LOG_FILENAME) : null;
  ...
  return {
    ...
    async logConfig(config, source) {
      ...
      const sanitizedConfig = {
        buildDir: path3.basename(config.buildDir),
  ...
  try {
    ...
    if (scanResult.errors.length > 0 && config.verbose) {
      ...
      for (const error of scanResult.errors) {
        console.log(chalk2.yellow(`  - ${error}`));
      }
      console.log();
    }
    ...
    console.log(chalk2.bold("\u{1F4CA} Analysis Summary\n"));
    console.log(`  Files scanned:        ${chalk2.cyan(scanResult.files.length)}`);
    console.log(`  Unique patterns:      ${chalk2.cyan(summary.totalPatterns)}`);
    console.log(`  Total occurrences:    ${chalk2.cyan(summary.totalOccurrences)}`);
    console.log(`  Avg. frequency:       ${chalk2.cyan(summary.avgFrequency.toFixed(1))}`);
    console.log(`  Avg. classes/pattern: ${chalk2.cyan(summary.avgClassesPerPattern.toFixed(1))}`);
    console.log();
    console.log(chalk2.bold("\u{1F4B0} Potential Savings\n"));
    console.log(`  Patterns to consolidate: ${chalk2.green(summary.totalPatterns)}`);
    console.log(`  Estimated savings:       ${chalk2.green(formatBytes(summary.totalBytesSaved))}`);
    console.log();
    if (candidates.length > 0) {
      console.log(chalk2.bold("\u{1F51D} Top 10 Consolidation Candidates\n"));
      const top10 = candidates.slice(0, 10);
      for (let i = 0; i < top10.length; i++) {
        const c = top10[i];
        console.log(
          `  ${chalk2.gray(`${i + 1}.`)} ${chalk2.yellow(c.hashName)} ${chalk2.gray("(")}${chalk2.cyan(c.frequency)}x${chalk2.gray(")")} saves ${chalk2.green(formatBytes(c.bytesSaved))}`
        );
        console.log(`     ${chalk2.gray(truncate(c.classString, 60))}`);
        if (c.excludedClasses.length > 0) {
          console.log(`     ${chalk2.gray("excluded:")} ${chalk2.yellow(c.excludedClasses.join(" "))}`);
        }
      }
      console.log();
    }
    if (candidates.length === 0) {
      console.log(chalk2.yellow("No patterns found that meet the consolidation criteria."));
      console.log(chalk2.gray("Try lowering --min-occurrences or --min-classes"));
    } else {
      console.log(chalk2.gray(`Run ${chalk2.cyan("classpresso optimize")} to apply these optimizations.`));
    }
    if (config.debug && logger.getLogPath()) {
      console.log(chalk2.gray(`Debug log: ${logger.getLogPath()}`));
    }
    console.log();
    await logger.close();
  } catch (err) {
...
async function saveMappingManifest(mappings, metrics, config) {
  ...
  const manifestPath = path3.join(config.buildDir, "classpresso-manifest.json");
  await writeFileContent(manifestPath, JSON.stringify(manifest, null, 2));
  return manifestPath;
}
async function loadMappingManifest(buildDir) {
  const manifestPath = path3.join(buildDir, "classpresso-manifest.json");
  try {
...
async function optimizeCommand(options) {
  ...
  if (dryRun) {
    console.log(chalk2.yellow("\u{1F50D} DRY RUN MODE - No files will be modified\n"));
  }
  if (config.ssr) {
    console.log(chalk2.blue("\u{1F512} SSR MODE - Only transforming patterns found in both HTML and JS\n"));
  }
  try {
    await logger.logSystemInfo();
    await logger.logConfig(config, "optimize command");
    console.log(chalk2.gray("Scanning build output..."));
    const scanStartTime = Date.now();
    ...
    console.log(chalk2.green(`  \u2713 Scanned ${scanResult.files.length} files
`));
    console.log(chalk2.gray("Detecting patterns..."));
    const detectStartTime = Date.now();
    ...
    console.log(chalk2.green(`  \u2713 Found ${candidates.length} patterns to consolidate
`));
    if (candidates.length === 0) {
      console.log(chalk2.yellow("No patterns found that meet the consolidation criteria."));
      console.log(chalk2.gray("Try lowering --min-occurrences or --min-classes\n"));
    ...
    console.log(chalk2.gray("Creating class mappings..."));
    ...
    console.log(chalk2.green(`  \u2713 Created ${mappings.length} mappings
`));
    console.log(chalk2.gray("Generating consolidated CSS..."));
    ...
    console.log(chalk2.green(`  \u2713 Generated ${formatBytes(cssBytes)} of CSS
`));
    console.log(chalk2.gray("Transforming build output..."));
    ...
    console.log(chalk2.green(`  \u2713 Modified ${transformResult.filesModified} files
`));
    if (config.verbose) {
      if (scanResult.dynamicBasePatterns.size > 0) {
        console.log(chalk2.gray(`  \u2139 Found ${scanResult.dynamicBasePatterns.size} dynamic class patterns (template literals)
`));
      }
      if (scanResult.mergeablePatterns.size > 0) {
        console.log(chalk2.gray(`  \u2139 Found ${scanResult.mergeablePatterns.size} mergeable patterns (className props)
`));
      }
    }
    if (!dryRun) {
      console.log(chalk2.gray("Injecting consolidated CSS..."));
      ...
      console.log(chalk2.green(`  \u2713 Injected into ${cssFile}
`));
    }
    let purgeBytesSaved = 0;
    if (config.purgeUnusedCSS && !dryRun) {
      console.log(chalk2.gray("Purging unused CSS..."));
      ...
      console.log(chalk2.green(`  \u2713 Purged ${purgeResult.rulesRemoved} unused CSS rules (${formatBytes(purgeResult.bytesSaved)} saved)
`));
    ...
    if (!dryRun && config.manifest) {
      console.log(chalk2.gray("Saving manifest..."));
      ...
      console.log(chalk2.green(`  \u2713 Saved to ${manifestPath}
`));
    }
...
\u2554${borderChar.repeat(width)}\u2557`));
  console.log(chalk2.cyan(`\u2551${" ".repeat((width - title.length) / 2)}${title}${" ".repeat((width - title.length) / 2)}\u2551`));
  console.log(chalk2.cyan(`\u2560${borderChar.repeat(width)}\u2563`));
  const lines = [
  ...
  for (const [label, value] of lines) {
    if (label === "") {
      ...
      console.log(chalk2.cyan(`\u2551 ${chalk2.bold(label)}${" ".repeat(padding)} \u2551`));
    } else {
      const padding = width - label.length - value.length - 3;
      console.log(chalk2.cyan(`\u2551 ${label}${" ".repeat(Math.max(1, padding))}${chalk2.green(value)} \u2551`));
    }
  }
  console.log(chalk2.cyan(`\u2560${borderChar.repeat(width)}\u2563`));
  console.log(chalk2.cyan(`\u2551 TOP CONSOLIDATIONS${" ".repeat(width - 20)} \u2551`));
  for (let i = 0; i < Math.min(5, metrics.topConsolidations.length); i++) {
    ...
    console.log(chalk2.cyan(`\u2551${line}${" ".repeat(Math.max(1, padding))}\u2551`));
  }
  console.log(chalk2.cyan(`\u255A${borderChar.repeat(width)}\u255D
`));
}
async function reportCommand(options) {
  ...
  if (!manifest) {
    console.error(chalk2.red("No classpresso manifest found in build directory."));
    console.log(chalk2.gray('Run "classpresso optimize" first to generate a manifest.'));
    process.exit(1);
  }
...
function printTextReport(manifest) {
  ...
  console.log(chalk2.cyan("\n\u2615 CLASSPRESSO REPORT\n"));
  console.log(chalk2.gray(`Generated: ${manifest.created}`));
  console.log(chalk2.gray(`Build: ${manifest.buildDir}
`));
  console.log(chalk2.bold("\u{1F4CA} Summary\n"));
  console.log(`  Files scanned:     ${chalk2.cyan(metrics.totalFilesScanned)}`);
  console.log(`  Files modified:    ${chalk2.cyan(metrics.totalFilesModified)}`);
  console.log(`  Patterns found:    ${chalk2.cyan(metrics.uniqueClassPatterns)}`);
  console.log(`  Patterns applied:  ${chalk2.cyan(metrics.consolidatedPatterns)}`);
  console.log();
  console.log(chalk2.bold("\u{1F4BE} Size Impact\n"));
  console.log(`  Bytes saved:       ${chalk2.green(formatBytes(metrics.bytesSaved))}`);
  console.log(`  CSS overhead:      ${chalk2.yellow(formatBytes(metrics.consolidatedCSSBytes))}`);
  console.log(`  Net reduction:     ${chalk2.green(formatBytes(metrics.bytesSaved - metrics.consolidatedCSSBytes))}`);
  console.log(`  Percentage:        ${chalk2.green(formatPercentage(metrics.percentageReduction))}`);
  console.log();
  console.log(chalk2.bold("\u26A1 Browser Impact\n"));
  console.log(`  Parse time saved:  ${chalk2.green(formatTime(metrics.estimatedParseTimeSavedMs))}`);
  console.log(`  Render time saved: ${chalk2.green(formatTime(metrics.estimatedRenderTimeSavedMs))}`);
  console.log();
  if (metrics.topConsolidations.length > 0) {
    console.log(chalk2.bold("\u{1F51D} Top Consolidations\n"));
    for (let i = 0; i < metrics.topConsolidations.length; i++) {
      const c = metrics.topConsolidations[i];
      console.log(
        `  ${chalk2.gray(`${i + 1}.`)} ${chalk2.yellow(c.consolidated)} ${chalk2.gray("(")}${chalk2.cyan(c.frequency)}x${chalk2.gray(")")} saves ${chalk2.green(formatBytes(c.bytesSaved))}`
      );
    }
    console.log();
  }
  console.log(chalk2.bold("\u{1F4CB} All Mappings\n"));
  for (const mapping of manifest.mappings.slice(0, 20)) {
    console.log(`  ${chalk2.yellow(mapping.consolidated)} \u2190 ${chalk2.gray(truncate2(mapping.original, 50))}`);
  }
  if (manifest.mappings.length > 20) {
    console.log(chalk2.gray(`  ... and ${manifest.mappings.length - 20} more`));
  }
  console.log();
}
